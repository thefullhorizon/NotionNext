...
async function convertNotionToSiteData(
  ...
  const sourceIcon = NOTION_CONFIG.AVATAR || collection?.icon
  const icon = sourceIcon?.startsWith('notion://')
    ? await resolveSiteIcon(sourceIcon, pageRecordMap)
    : NOTION_CONFIG.AVATAR
  const siteInfo = getSiteInfo({ collection, block, NOTION_CONFIG, icon })
...
function getSiteInfo({ collection, block, NOTION_CONFIG, icon: resolvedIcon }) {
